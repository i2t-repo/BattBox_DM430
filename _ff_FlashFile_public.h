...
/********************************************************
 File        : 	_ff_FlashFile_public.h
   ...
   04.31  06May11   fbjorn    Added SeekLn and Read functionallity (copied from DP200)
...
#define _ff_SectorHeadderType   1U
#define _ff_InitSectorHeadder   { (U16)_ff_SectorHeadderType, (U16)_Bytesizeof(ff_SECTOR_HEADDER_TYPE), (U16)0U }
...
#define _ff_CREATE_FILE_SECTOR(addr, size)    { addr, size, 0, 0, _ff_InitSectorHeadder }
#define _ff_CREATE_FILE_END(name, filetype)   };                                                                  \
   static ff_FILE_INFO_TYPE name##_i = {0xFFFFFFFFUL,0xFFFFFFFFUL,0,0,0,0,0,0,0,0,filetype,0,_NrOfFileSectors(name),name##_s}; \
...
typedef struct
{
   U32 address;
   U32 size;
   U32 spos;                        // Sector Pos as Data length pos
   U32 nrecords;                    // number of records in this sector
...
typedef struct
{
   U32 recsize;     		// Save recpos when the file is closed after append or write
   U32 fsize;				// Saves fpos when the file is closed after append or write
   U32 recpos;      		// actual record position in file
   U16 status;              // File Status ( Open, Closed ...)
   U32 fpos;                // File Pos as Data length pos
   U32 soffs;               // File pos as Sector offset
...
********************************************************/
#ifndef  _WriteOnly
/*lint -esym(757, ff_Read) */	// disable warning "global declarator not referenced"
...
********************************************************/
#ifndef  _WriteOnly
/*lint -esym(757, ff_Readln) */	// disable warning "global declarator not referenced"
U16 ff_Readln(ff_FILE *f,U8 *dst,U16 maxlen,U16 *lread);
#endif
...
********************************************************/
#ifndef  _WriteOnly
/*lint -esym(757, ff_Seek) */	// disable warning "global declarator not referenced"
...
********************************************************/
#ifndef  _WriteOnly
/*lint -esym(757, ff_Seekln) */	// disable warning "global declarator not referenced"
U16 ff_Seekln(ff_FILE *f,U32 pos);
#endif
